...
- Qwen 3.6 27B full-checkpoint OBLITERATUS has been tried and is no longer the
  ...
  `configs/abliteration/qwen36_27b_ft_abli_v2_native_sra_v51.yaml`. V52 then
  changed the direction basis to SOM residual centroids while retaining the
  generic native SRA checkpoint workflow:
  `configs/abliteration/qwen36_27b_ft_abli_v2_native_som_sra_v52.yaml`. It
  exported, synced, audited, served, and targeted-gated, but is rejected because
  refusal wording remained 1/3. Do not broad-eval, quantize, upload, promote,
  or rerun V52 unchanged.
  ...
  `model_selection_summary` to 2/3. V40 has now been attempted and rejected:
  ...
  embeddings, `lm_head`, MLP weights, routers, and experts untouched. It
  exported, changed 10 tensors, synced to the worker, passed strict local and
  worker audits, served TP=2, and completed the targeted gate. Reject it:
...
8. Continue Qwen 3.6 FT-ablation search from the promoted local FT v4 source.
   ...
   V30 source-tethered OBLITERATUS has now been attempted and is blocked:
   ...
   remap and alpha 0.895/top-43 source tether toward local FT v4. The guarded
   run loaded all 851 Qwen weights, then host MemAvailable fell below the 5%
   ...
   V31 generated-token selective projection has now been attempted and
   rejected:
   ...
   preservation. The checkpoint exported safely, changed 20 tensors, passed
   ...
   stronger 96-token prefix unlikelihood. The guarded two-Spark run completed,
   merged, synced to the worker, passed strict local and worker audits, served
   ...
   to 2/3. V52 replaced the mean/SVD refusal basis with SOM residual centroids
   ...
   V22 implemented a method shift, but its checked gate evidence rejects it. Do
   not retry the same selective top-k/layer-window recipe unchanged.
