...
if [[ -n "${MODEL_FORGE_SPARK_ETH_IF:-}" && "${VLLM_SPARK_EXTRA_DOCKER_ARGS:-}" != *"NCCL_SOCKET_IFNAME="* ]]; then
  export VLLM_SPARK_EXTRA_DOCKER_ARGS="${VLLM_SPARK_EXTRA_DOCKER_ARGS:-} -e NCCL_SOCKET_IFNAME=${MODEL_FORGE_SPARK_ETH_IF}"
fi
