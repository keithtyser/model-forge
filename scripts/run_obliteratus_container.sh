...
RUNNER="${1:?usage: scripts/run_obliteratus_container.sh <generated-run_obliteratus.py>}"
IMAGE="${MODEL_FORGE_OBLITERATUS_IMAGE:-model-forge-obliteratus:latest}"
MODELS_DIR="${MODEL_FORGE_MODELS_DIR:-$HOME/models}"
HF_CACHE="${HF_HOME:-$HOME/.cache/huggingface}"

if [[ ! -f "$RUNNER" ]]; then
  echo "[model-forge] missing OBLITERATUS runner: $RUNNER" >&2
  exit 2
fi

TOTAL_MEM_GB="${MODEL_FORGE_OBLITERATUS_DOCKER_MEMORY_GB:-$(awk '/MemTotal/ {printf "%d", ($2 / 1024 / 1024) * 0.85}' /proc/meminfo)}"
CPU_LIMIT="${MODEL_FORGE_OBLITERATUS_DOCKER_CPUS:-$(python3 - <<'PY'
import os
...
WATCHDOG_INTERVAL="${MODEL_FORGE_OBLITERATUS_HOST_WATCHDOG_INTERVAL_SECONDS:-10}"
CONTAINER_NAME="${MODEL_FORGE_OBLITERATUS_CONTAINER_NAME:-model-forge-obliteratus-$(id -u)-$$}"
WATCHDOG_FLAG="$(mktemp "${TMPDIR:-/tmp}/model-forge-obliteratus-watchdog.XXXXXX")"
...
if total <= 0 or available / total < floor:
    raise SystemExit(
        f"[model-forge] refusing to start OBLITERATUS: host MemAvailable "
...
docker run --rm --gpus all \
  ...
  --shm-size="${MODEL_FORGE_OBLITERATUS_SHM_SIZE:-32g}" \
  --pids-limit="${MODEL_FORGE_OBLITERATUS_PIDS_LIMIT:-4096}" \
  --user "$(id -u):$(id -g)" \
