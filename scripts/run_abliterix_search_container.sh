...
RUNNER="${1:?usage: scripts/run_abliterix_search_container.sh <generated-run_abliterix_search.py>}"
IMAGE="${MODEL_FORGE_ABLITERIX_IMAGE:-model-forge-abliterix:latest}"
MODELS_DIR="${MODEL_FORGE_MODELS_DIR:-$HOME/models}"
HF_CACHE="${HF_HOME:-$HOME/.cache/huggingface}"

if [[ ! -f "$RUNNER" ]]; then
  echo "[model-forge] missing Abliterix runner: $RUNNER" >&2
  exit 2
fi

TOTAL_MEM_GB="${MODEL_FORGE_ABLITERIX_DOCKER_MEMORY_GB:-$(awk '/MemTotal/ {printf "%d", ($2 / 1024 / 1024) * 0.85}' /proc/meminfo)}"
...
WATCHDOG_FLAG="$(mktemp "${TMPDIR:-/tmp}/model-forge-abliterix-watchdog.XXXXXX")"
...
if total <= 0 or available / total < floor:
    raise SystemExit(
        f"[model-forge] refusing to start Abliterix: host MemAvailable "
...
docker run --rm --gpus all \
  ...
  --shm-size="${MODEL_FORGE_ABLITERIX_SHM_SIZE:-32g}" \
  --pids-limit="${MODEL_FORGE_ABLITERIX_PIDS_LIMIT:-4096}" \
  --user "$(id -u):$(id -g)" \
