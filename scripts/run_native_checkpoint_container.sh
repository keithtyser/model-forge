...
RUNNER="${1:?usage: scripts/run_native_checkpoint_container.sh <generated-runner.py>}"
IMAGE="${MODEL_FORGE_NATIVE_CHECKPOINT_IMAGE:-model-forge-posttrain-tf5:latest}"
...
TOTAL_MEM_GB="${MODEL_FORGE_NATIVE_CHECKPOINT_DOCKER_MEMORY_GB:-$(awk '/MemTotal/ {printf "%d", ($2 / 1024 / 1024) * 0.85}' /proc/meminfo)}"
CPU_LIMIT="${MODEL_FORGE_NATIVE_CHECKPOINT_DOCKER_CPUS:-$(python3 - <<'PY'
import os
...
WATCHDOG_INTERVAL="${MODEL_FORGE_NATIVE_CHECKPOINT_HOST_WATCHDOG_INTERVAL_SECONDS:-10}"
CONTAINER_NAME="${MODEL_FORGE_NATIVE_CHECKPOINT_CONTAINER_NAME:-model-forge-native-checkpoint-$(id -u)-$$}"
WATCHDOG_FLAG="$(mktemp "${TMPDIR:-/tmp}/model-forge-native-checkpoint-watchdog.XXXXXX")"
...
if total <= 0 or available / total < floor:
    raise SystemExit(
        f"[model-forge] refusing to start native checkpoint: host MemAvailable "
...
docker run --rm --gpus all \
  ...
  --shm-size="${MODEL_FORGE_NATIVE_CHECKPOINT_SHM_SIZE:-32g}" \
  --pids-limit="${MODEL_FORGE_NATIVE_CHECKPOINT_PIDS_LIMIT:-4096}" \
  --user "$(id -u):$(id -g)" \
