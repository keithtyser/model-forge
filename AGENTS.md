...
It was attempted and still crossed the 5% host RAM floor before writing output,
because upstream OBLITERATUS activation collection hooks every layer before
adapter materialization. Do not rerun V47 unchanged.

V48 keeps the same late-layer target set, but the generated model-forge runner
also patches OBLITERATUS activation collection so hooks are installed only on
those target layers. It uses one refusal direction and rank-1 LoRA to keep the
first target-layer activation retry small:
...
It exported, synced, passed strict local and worker audits, served TP=2, and
completed the targeted gate. Reject it because safe redirect,
harmful-detail/compliance, and `model_selection_summary` all passed 3/3, but
...
and `model_selection_summary`, but still used denial/meta openings. It uses
...
harmful-vs-benign prompts, measure hidden states at the first generated
assistant token under the chat template, project the harmful direction against
benign means, preserve row norms, and export a normal sharded checkpoint. V31
selects the highest-separation middle/late layers and edits only output
projection tensors (`self_attn.o_proj`, `linear_attn.out_proj`, and
...
`mean_plus_paired_svd`, narrows harmful prompts to the observed self-harm
refusal-opening family, adds explicit model-selection preservation anchors, and
edits only attention output projections (`self_attn.o_proj` and
`linear_attn.out_proj`) in layers `20-47`. It intentionally avoids embeddings,
...
The Qwen V20 branch is
`configs/abliteration/qwen36_27b_ft_abli_v2_hybrid_attention_som_projection_v20.yaml`.
It registers `local_ft_abli_som_projection_v20_hybrid_attention`. V20 was
intentionally not another prompt-weight or scalar-strength bump: checkpoint
inspection showed this Qwen 3.6 model has both full-attention
`self_attn.o_proj.weight` tensors and hybrid `linear_attn.out_proj.weight`
tensors. V17 edited only the full-attention output path and got closest. V20
kept V17's refusal signal and SOM shape, then added a lower-weight
linear-attention output component. It exported successfully, changed 28 intended
attention-output tensors, synced to both Sparks, passed strict
checkpoint/tokenizer/architecture audits on coordinator and worker, served with
TP=2 after one transient NCCL retry, and ran the targeted three-trial gate.
Reject it: refusal wording stayed 1/3, ablation refusal suppression was 2/3,
safe redirect was 3/3, harmful detail/compliance and unsafe overcompliance were
0/3, and `model_selection_summary` was 3/3. See
...
finetune data prep accepted 150 rows. The 112-step guarded two-Spark training
run completed, the adapter merged to
`~/models/Qwen3.6-27B-local-ft-v4-abliterated-heretic-residual-trial12-refusal-pref-ul-v16-sampled-gate-repair`,
the model synced to the worker Spark, strict checkpoint/tokenizer/architecture
audits passed locally and on the worker, and TP=2 serving completed the targeted
gate. Result: self-harm refusal wording 1/3, safe redirect 3/3, harmful
detail/compliance 0/3, and `model_selection_summary` 3/3. Do not broad-eval,
...
V39/V17 direct-opening rewrite has now been attempted and rejected:
...
The guarded two-Spark training run completed 72 steps with train loss 13.11,
merged to
...
audits locally and on the worker, and served TP=2 for the targeted gate.
Result: self-harm refusal wording 1/3, safe redirect 3/3, harmful
detail/compliance 0/3, and `model_selection_summary` 2/3. Do not broad-eval,
NVFP4-export, upload, promote, or rerun V39 unchanged. The next candidate must
be materially different and capability-anchored, not another stronger
direct-opening rewrite. See
`reports/qwen36_27b_trial12_pref_ul_v17_direct_opening_rewrite_plan_summary.md`.

Current Qwen FT-abli state:
`score_distilled_repair_v44` has been trained and rejected in
`configs/abliteration/qwen36_27b_ft_abli_v2_candidate_gate.yaml`.

Do not rerun V44 unchanged. It completed guarded two-Spark training, merge,
sync, strict local/worker audits, TP=2 serving, and the targeted gate. Result:
self-harm refusal wording `3/3`, safe redirect `3/3`, harmful detail/compliance
`0/3`, and `model_selection_summary` `3/3`. Do not broad-eval, quantize,
upload, or promote it.
...
constants. The negative result matters: scorer-distilled preference repair
